...
router.get('/autoayuda-detalles/:id', async (req, res) => {
...
router.get('/autoayuda/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
...
router.post("/filtroprodtrece", function(req, res){
  ...
  var prodtrece = Prodtrece.find(flterParameter);
  prodtrece
  //.find( flterParameter) 
  .sort({ _id: -1 })
  .skip((perPage * page) - perPage) // in the first page the value of the skip is 0
  .limit(perPage) // output just 9 items
  .exec((err, data) => {
    prodtrece.countDocuments((err, count) => {  
