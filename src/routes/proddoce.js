...
router.post('/proddoce/new-proddoce',  async (req, res) => {
  const { 
    name,
    title,
    image,
...
router.get('/esotericos-detalles/:id', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);

  const { id } = req.params;
  const proddoce = await Proddoce.findById(id);
  res.render('proddoce/proddoceredirect', {
    proddoce,
...
router.get('/esotericos/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
  ...
  .exec((err, proddoce) => {
    Proddoce.countDocuments((err, count) => { // count to calculate the number of pages
...
router.get('/proddoceback/:page', async (req, res) => {
  ...
  .exec((err, proddoce) => {
    Proddoce.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('proddoce/new-proddoce', {
        proddoce,
        current: page,
        pages: Math.ceil(count / perPage)
      });
...
router.get('/proddoce/edit/:id',  async (req, res) => {
  const proddoce = await Proddoce.findById(req.params.id);
  res.render('proddoce/edit-proddoce', { proddoce });
});

router.post('/proddoce/edit/:id',  async (req, res) => {
  const { id } = req.params;
...
// Delete 
router.get('/proddoce/delete/:id', async (req, res) => {
  const { id } = req.params;
    await Proddoce.deleteOne({_id: id});
  res.redirect('/proddoceback/:1');
});


router.get('/likeproddoce/:id', async (req, res, next) => {
  // let { id } = req.params;
  // const task = await Ofertauno.findById(id);
  const task = await Proddoce.findById(req.params.id);
  task.like = !task.like;
  await task.save();
...
router.get('/addtocardproddoce/:id', function(req, res, next){
  ...
  Proddoce.findById(productId,async function(err, product){
    if(err){
      return res-redirect('/');
    }
