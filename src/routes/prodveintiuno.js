...
// Models
const Prodveintiuno = require('../models/prodveintiuno');
...
router.post('/prodveintiuno/new-prodveintiuno',  async (req, res) => {
  const { 
    name,
    title,
    image,
  ...
  if (errors.length > 0) {
    ...
    const newNote = new Prodveintiuno({ 
        name,
        title,
        image,
        imagedos,
...
router.get('/prodveintiunoredirect/:id', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);

  const { id } = req.params;
  const prodveintiuno = await Prodveintiuno.findById(id);
  res.render('prodveintiuno/prodveintiunoredirect', {
    prodveintiuno,
...
router.get('/prodveintiunoindex/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
  ...
  .exec((err, prodveintiuno) => {
    Prodveintiuno.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('prodveintiuno/prodveintiuno', {
        prodveintiuno,
        current: page,
...
router.get('/prodveintiunoback/:page', async (req, res) => {
  let perPage = 8;
  let page = req.params.page || 1;
  ...
  .exec((err, prodveintiuno) => {
    Prodveintiuno.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('prodveintiuno/new-prodveintiuno', {
        prodveintiuno,
        current: page,
        pages: Math.ceil(count / perPage)
      });
...
router.get('/prodveintiuno/edit/:id',  async (req, res) => {
  const prodveintiuno = await Prodveintiuno.findById(req.params.id);
  res.render('prodveintiuno/edit-prodveintiuno', { prodveintiuno });
});
...
// Delete 
router.get('/prodveintiuno/delete/:id', async (req, res) => {
  const { id } = req.params;
    await Prodveintiuno.deleteOne({_id: id});
