...
router.get('/decoracion/:page', async (req, res) => {
...
router.get('/decoracion-detalles/:id', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
...
router.post("/filtroproddiecinueve", function(req, res){
  ...
  var proddiecinueve = Proddiecinueve.find(flterParameter);
  proddiecinueve
  //.find( flterParameter) 
  .sort({ _id: -1 })
  .skip((perPage * page) - perPage) // in the first page the value of the skip is 0
  .limit(perPage) // output just 9 items
  .exec((err, data) => {
    proddiecinueve.countDocuments((err, count) => {  
