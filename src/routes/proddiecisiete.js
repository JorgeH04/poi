...
router.post('/proddiecisiete/new-proddiecisiete',  async (req, res) => {
  const { 
    name,
    title,
    image,
  ...
  if (errors.length > 0) {
    ...
    const newNote = new Proddiecisiete({ 
      name,
      title,
      image,
      imagedos,
    ...
    res.redirect('/proddiecisieteback/1');
  }
});
...
router.get('/empresariales-detalles/:id', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);

  const { id } = req.params;
  const proddiecisiete = await Proddiecisiete.findById(id);
  res.render('proddiecisiete/proddiecisieteredirect', {
    proddiecisiete,
...
router.get('/empresariales/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
  ...
  .exec((err, proddiecisiete) => {
    Proddiecisiete.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('proddiecisiete/proddiecisiete', {
        proddiecisiete,
        current: page,
...
router.get('/likeproddiecisiete/:id', async (req, res, next) => {
  // let { id } = req.params;
  // const task = await Ofertauno.findById(id);
  const task = await Proddiecisiete.findById(req.params.id);
  task.like = !task.like;
  await task.save();
...
router.get('/proddiecisieteback/:page', async (req, res) => {
  ...
  .exec((err, proddiecisiete) => {
    Proddiecisiete.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('proddiecisiete/new-proddiecisiete', {
        proddiecisiete,
        current: page,
        pages: Math.ceil(count / perPage)
      });
...
router.post("/filtroproddiecisiete", function(req, res){
  ...
  var proddiecisiete = Proddiecisiete.find(flterParameter);
  proddiecisiete
  //.find( flterParameter) 
  .sort({ _id: -1 })
  .skip((perPage * page) - perPage) // in the first page the value of the skip is 0
  .limit(perPage) // output just 9 items
  .exec((err, data) => {
    proddiecisiete.countDocuments((err, count) => {  
...
// Delete 
router.get('/proddiecisiete/delete/:id', async (req, res) => {
  const { id } = req.params;
    await Proddiecisiete.deleteOne({_id: id});
  res.redirect('/proddiecisieteback/:1');
});
