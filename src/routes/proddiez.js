...
router.post('/proddiez/new-proddiez',  async (req, res) => {
  const { 
    name,
    title,
    image,
...
router.get('/psicologia-detalles/:id', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);

  const { id } = req.params;
  const proddiez = await Proddiez.findById(id);
  res.render('proddiez/proddiezredirect', {
    proddiez,
...
router.get('/psicologia/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
  ...
  .exec((err, proddiez) => {
    Proddiez.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('proddiez/proddiez', {
        proddiez,
        current: page,
...
router.post("/filtroproddiez", function(req, res){
  ...
  var proddiez = Proddiez.find(flterParameter);
  proddiez
  //.find( flterParameter) 
  .sort({ _id: -1 })
  .skip((perPage * page) - perPage) // in the first page the value of the skip is 0
  .limit(perPage) // output just 9 items
  .exec((err, data) => {
    proddiez.countDocuments((err, count) => {  
...
router.get('/proddiezback/:page', async (req, res) => {
  let perPage = 8;
  let page = req.params.page || 1;
  ...
  .exec((err, proddiez) => {
    Proddiez.countDocuments((err, count) => { // count to calculate the number of pages
      if (err) return next(err);
      res.render('proddiez/new-proddiez', {
        proddiez,
        current: page,
        pages: Math.ceil(count / perPage)
      });
...
router.get('/proddiez/edit/:id',  async (req, res) => {
  const proddiez = await Proddiez.findById(req.params.id);
  res.render('proddiez/edit-proddiez', { proddiez });
});

router.post('/proddiez/edit/:id',  async (req, res) => {
  const { id } = req.params;
...
// Delete 
router.get('/proddiez/delete/:id', async (req, res) => {
  const { id } = req.params;
    await Proddiez.deleteOne({_id: id});
  res.redirect('/proddiezback/:1');
});



router.get('/likeproddiez/:id', async (req, res, next) => {
  // let { id } = req.params;
  // const task = await Ofertauno.findById(id);
  const task = await Proddiez.findById(req.params.id);
  task.like = !task.like;
  await task.save();
...
router.get('/addtocardproddiez/:id', function(req, res, next){
  var productId = req.params.id;
  var cart = new Cart(req.session.cart ? req.session.cart : {items: {}});
  var cartdolar = new Cartdolar(req.session.cartdolar ? req.session.cartdolar : {items: {}});
  Proddiez.findById(productId,async function(err, product){
    if(err){
      return res-redirect('/');
    }
