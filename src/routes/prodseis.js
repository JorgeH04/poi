...
router.get('/literatura-latinoamericana-detalles/:id', async (req, res) => {
...
router.get('/literatura-latinoamericana/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
...
router.post("/filtroprodseis", function(req, res){
  ...
  var prodseis = Prodseis.find(flterParameter);
  prodseis
  //.find( flterParameter) 
  .sort({ _id: -1 })
  .skip((perPage * page) - perPage) // in the first page the value of the skip is 0
  .limit(perPage) // output just 9 items
  .exec((err, data) => {
    prodseis.countDocuments((err, count) => {  
