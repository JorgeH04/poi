...
router.get('/literatura-infantil-detalles/:id', async (req, res) => {
...
router.get('/literatura-infantil/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
...
router.post("/filtroprodsiete", function(req, res){
  ...
  var prodsiete = Prodsiete.find(flterParameter);
  prodsiete
  //.find( flterParameter) 
  .sort({ _id: -1 })
  .skip((perPage * page) - perPage) // in the first page the value of the skip is 0
  .limit(perPage) // output just 9 items
  .exec((err, data) => {
    prodsiete.countDocuments((err, count) => {  
