...
router.get('/historia-politica-detalles/:id', async (req, res) => {
...
router.get('/historia-politica/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
...
router.get('/addtocardproddieciseis/:id', function(req, res, next){
  ...
  Proddieciseis.findById(productId,async function(err, product){
    if(err){
      return res-redirect('/');
    }
